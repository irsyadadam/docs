...
We can now use the embeddings (`X`) and the **labels** table to train multiple types of task heads to predict clinical outcomes. After importing the necessary packages and splitting our data into training and test sets, we complete Task A: a binary classifier to predict subject readmission to the hospital:

| Task | Data Column(s)                            | Description                                                       | |
| ---- | ----------------------------------------- | ----------------------------------------------------------------- | --- |
| A    | `readmission_risk`                        | Binary classifier to predict subject readmission to the hospital  | |
| B    | `phenotype_class`                         | Multi-class classifier to predict cancer stage                    | |
| C    | `overall_survival_months`                 | Regression model to predict months survival for subjects who died | |
| D    | `overall_survival_months; event_observed` | Cox proportional hazard between a fictional variable and death    | |
