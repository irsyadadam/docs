...
<Info>
  Note that smb-v1-1.7B only supports a context length of 3k, yet many patient histories exceed this constraint.

  As a temporary solution to efficiently manage context, our [smb-utils](https://github.com/standardmodelbio/smb-utils) package offers functionality to 1) filter events data by modality (i.e., code or table columns), 2) organize events into time bins with the most recent events from an anchor date, or 3) flexibly define custom event categories.
...
Assume your events are saved as `internal_cohort_meds.parquet` with the scchema above. Load and verify, then serialize one patient:
