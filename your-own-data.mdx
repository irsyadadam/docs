...
1. **Format data:** Convert [MEDS (Medical Event Data Standard)](https://github.com/Medical-Event-Data-Standard/meds) formatted EHR data into serial tokens using `smb_utils`.
...
Make sure you've completed the [quickstart](/) setup. All commands below should be run from the `quickstart/` directory using `uv run`.
...
2. **The Modality (Text):** We provide `smb_utils` to deterministically convert your MEDS tables into a tokenizable XML-like stream to input to the model. This function handles date grouping, modality tagging (e.g., `<conditions>`), and causal masking.
